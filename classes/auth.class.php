...
final class auth
{
    ...
    public function get_authentication($auth_data)
    {
        ...
        {       
            $sql = "select * from store_access where store ='".$store_id."' and status='Y' and id = 2 ";
            $this->db->query($sql);
            ...
            $myshopify_domain = $decode_rslt->shop->myshopify_domain;

            $entered_domain = trim($store_id).".myshopify.com";
            
            if( strtolower($myshopify_domain) == strtolower($entered_domain) ){
